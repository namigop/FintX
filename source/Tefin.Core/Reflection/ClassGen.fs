...
module ClassGen =
    ...
    let private getTypeBuilder (assemblyName:string) (moduleName:string) (className:string) =
        ...
        let assemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(an, AssemblyBuilderAccess.Run)
        let moduleBuilder = assemblyBuilder.DefineDynamicModule(moduleName);
        let tb = moduleBuilder.DefineType(className,
    ...
    let private createGetMethod (myTypeBld:TypeBuilder) (methodName:string)  (returnType:Type) =
        let methodBuilder = myTypeBld.DefineMethod(methodName, MethodAttributes.Public, returnType, null)
    ...
    let private createProperty (tb:TypeBuilder) (propertyName:string) (propertyType:Type) =
        if not(typeof<unit> = propertyType) then
            ...
            let getPropMethodBuilder = tb.DefineMethod("get_" + propertyName, MethodAttributes.Public ||| MethodAttributes.SpecialName ||| MethodAttributes.HideBySig, propertyType, Type.EmptyTypes)
            let getIl = getPropMethodBuilder.GetILGenerator()

            getIl.Emit(OpCodes.Ldarg_0)
            getIl.Emit(OpCodes.Ldfld, fieldBuilder)
            getIl.Emit(OpCodes.Ret)
            let setPropMethodBuilder = tb.DefineMethod("set_" + propertyName,
    ...
    let create (assemblyName:string) (moduleName:string) (className:string) (targetProps: PropInfo array) =
        ...
        let _ = tb.DefineDefaultConstructor(MethodAttributes.Public ||| MethodAttributes.SpecialName ||| MethodAttributes.RTSpecialName)

        let targetMethods, targetProperties = targetProps |> Array.partition (fun t -> t.IsMethod)
