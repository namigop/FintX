...
module RequestUtils =
    ...
    let emitRequestClass  (methodInfo:MethodInfo) =
        ...
        let getProperties() =
           ...
           |> Array.map (fun pi -> {IsMethod = false; Name = pi.Name; Type = pi.ParameterType } )
    ...
    let emitStreamingRequestClass (prefix:string) (methodInfo:MethodInfo) =
        ...
        let getClassName() =
           let className = $"{prefix}_{methodInfo.Name}_{methodInfo.ReturnType.Name}_{requestType.Name}_{methodInfo.GetHashCode()}";
           className
