...
module CallPipeline =
    ...
    let start() =
        { StartTime = DateTime.Now
          ...
          Io = None
          Variable = Dictionary<string, obj>() }
    let exec (steps: Step array) (execContext:Context) = task {

        //Execute 1 step
        let execStep (step:Step) (ctx:Context) = task {
            let sw = Stopwatch.StartNew()
            try
                let! ctx2 = step.Run(ctx)
                sw.Stop()
                return {ctx2 with Success = true
                                  Steps = [step.Name, sw.Elapsed] |> List.append ctx.Steps }
            with
            | exc ->
                sw.Stop()
                return
                  {ctx with Response =  Res.failed exc
                            Success = false
                            Steps = [step.Name, sw.Elapsed] |> List.append ctx.Steps }}
