...
module ProtoDiscoveryClient =
    ...
    let private validateProtos (io: IOResolver) (protos : string array)=
         let invalidEntries =
                protos
                |> Array.filter (fun location ->
                                       not (location.EndsWith("proto") && io.File.Exists(location)))
         if invalidEntries.Length > 0 then
            Ret.Error (failwith $"{invalidEntries[0]} is invalid")
         else
            Ret.Ok protos

    let private generateSourceFiles (io: IOResolver) (discoParams: ProtoDiscoverParameters) (protos : string array) = task {
        let! files =
            ProtocProcess.generateSourceFiles
                io
                discoParams.Config["RootPath"]
                discoParams.Config["ProtosPath"]
                protos
