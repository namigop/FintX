...
type CallInterceptor(clientName: string, io: IOResolver) =
    ...
    let getWrappedTrailer_ClientStream (service: string) (method: string) (resp: AsyncClientStreamingCall<'TReq, 'TResp>) =
        let name = "GetTrailers"
        let onSuccessOpt = Some(onSuccess name)
        let onErrorOpt = Some(onError name)
    ...
    let getWrappedTrailer_DuplexStream (service: string) (method: string) (resp: AsyncDuplexStreamingCall<'TReq, 'TResp>) =
        let name = "GetTrailers"
        let onSuccessOpt = Some(onSuccess name)
        let onErrorOpt = Some(onError name)
    ...
    let getWrappedResponse_ClientStream (clientName: string) (method: string) (resp: AsyncClientStreamingCall<'TReq, 'TResp>) =
        task {
    ...
    override x.AsyncClientStreamingCall<'TReq, 'TResp when 'TReq: not struct and 'TResp: not struct>
        ...
        let newCall =
            new AsyncClientStreamingCall<'TReq, 'TResp>(writer, getResponseTask, getRespHeaderTask, getStatus, getTrailer, dispose)
