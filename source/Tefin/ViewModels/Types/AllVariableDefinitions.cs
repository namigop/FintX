...
public class AllVariableDefinitions {
        ...
        allVarDefs.RequestVariables.AddRange(
            allVars.RequestVariables
                ...
                .Select(v => new VarDefinition {
                    Tag = v.Tag,
                    TypeName = SystemType.getActualType(v.Type).Item2,
                    JsonPath = v.JsonPath,
                    Scope = v.Scope
                }));
    ...
    public static AllVariables To(AllVariableDefinitions allVars) {
        ...
        allVarDefs.RequestVariables.AddRange(
            allVars.RequestVariables.Select(v => new RequestEnvVar(v.Tag, v.JsonPath, v.TypeName, v.Scope)));
        allVarDefs.ResponseVariables.AddRange(
            allVars.ResponseVariables.Select(v => new RequestEnvVar(v.Tag, v.JsonPath, v.TypeName, v.Scope)));
        allVarDefs.RequestStreamVariables.AddRange(
            allVars.RequestStreamVariables.Select(v => new RequestEnvVar(v.Tag, v.JsonPath, v.TypeName, v.Scope)));
        allVarDefs.ResponseStreamVariables.AddRange(
            allVars.ResponseStreamVariables.Select(v => new RequestEnvVar(v.Tag, v.JsonPath, v.TypeName, v.Scope)));
        return allVarDefs;
    }
}