...
public class EnvVarNodeViewModel : ViewModelBase {
    ...
    public ITypeEditor? DefaultValueEditor {
        ...
        private set => this.RaiseAndSetIfChanged(ref this._defaultValueEditor, value);
    }

    public string EnvVarTag {
        get => string.IsNullOrWhiteSpace(this._envVarTag) ? "" : $"{{{{{this._envVarTag.ToUpperInvariant()}}}}}";
        set {
    ...
    public string SelectedScope {
        ...
        set => this.RaiseAndSetIfChanged(ref this._selectedScope, value);
    }
    ...
    public void CreateEnvVariable(string tag, string jsonPath, string? currentValue = null) {
        ...
        var nodeContainerVar =
            GetNodeContainerVar(this._node); // this._node.FindParentNode<NodeBase>(t => t.Parent == null);  
        var envTag = this.EnvVarTag.ToUpperInvariant();
        var currentVar = nodeContainerVar.Variables.FirstOrDefault(t => t.JsonPath == jsonPath);
        var isExistingNodeVar = currentVar != null;
    ...
    private void OnCancelEnvVariable() => this.EnvVarTag = this._ogTag;
    ...
    public void Reset() => this.EnvVarTag = this._ogTag;

    public void ShowDefault() {
        this.EnvVarTag = this._node.Title;
        var actualType = this._node.Type;
